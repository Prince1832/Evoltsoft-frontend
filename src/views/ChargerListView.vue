      ...
      <div
        ...
        <div class="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
          ...
          <button v-if="!showForm" @click="showAddForm"
            ...
            <span class="md:text-xl">+</span>Add Station
          </button>
        </div>
      </div>
...
export default {
  ...
  computed: {
    ...
    uniqueConnectorTypes() {
      const set = new Set(
        this.chargers
          .map(c => c.connectorType?.toString().trim())
          .filter(val => val)
      );
      return [...set];
    },
    filteredChargers() {
      return this.chargers.filter(charger => {
        const nameMatch = charger.name?.toLowerCase().includes(this.searchQuery.toLowerCase())
        const statusMatch = this.filters.status ? charger.status === this.filters.status : true
        const powerMatch = this.filters.power ? Number(charger.power) === this.filters.power : true
        const connectorMatch = this.filters.connectorType ? charger.connectorType === this.filters.connectorType : true
